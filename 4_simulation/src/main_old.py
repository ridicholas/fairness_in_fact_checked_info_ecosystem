...
beliefs = [{3: 0.5, 56: 0.5, 43: 0.5},
           {3: 0.35, 56: 0.65, 43: 0.65},
           {3: 0.65, 56: 0.35, 43: 0.65},
           {3: 0.65, 56: 0.65, 43: 0.35}]
...
# with open(config['output_sim_midpoint'] + '.pickle', 'wb') as file:
...
for rep in range(reps):
    ...
    # with open(config['output_sim_midpoint'] + '.pickle', 'rb') as file:
    ...
    check_pre.sample_claims(num_to_sample=config['claims_to_sample'], sample_method = 'nodes_visited')
    ...
    print('\n\n\n ----------- Sampling Claims for Checkworthy Dataset --------- \n\n\n')
    check_pre.sample_claims(num_to_sample=config['claims_to_sample'], sample_method='stratified_nodes_visited')

    print('\n\n\n ----------- Random Sampling of Labels for Checkworthy Dataset --------- \n\n\n')
    check_pre.sample_labels_for_claims(labels_per_claim = config['nodes_to_sample'], sample_method = 'knowledgable_community')

    print('\n\n\n ----------- Training Model with Label = average_truth_perception_kc --------- \n\n\n')
    ...
    sim.run(
        ...
        min_degree=config['min_degree'],
        fact_checks_per_step = config['fact_checks_per_step'],
        mitigation_type = 'TopPredictedByTopic'
    )
