...
check_df = pd.DataFrame.from_dict(check.checkworthy_data).T
...
def run_second_half(G, runtime, agg_interval=3, agg_steps=3, outcome_time=48, impactednesses = impactednesses, mitigation_type = "delete_from_inbox"):
    ...
    for step in bar(range(runtime)):
        ...
        ##for each time step, determine which claims are check-worthy
        check_df = pd.DataFrame.from_dict(check.checkworthy_data).T
        x = check_df[[i for i in check_df.columns if ('truth' not in i) and ('target' not in i) and ('step' not in i) and ('outcome' not in i) and ('value' not in i)]]
        preds = clf.predict(x)
        fact_checked = fact_checked + list(check_df.index[preds == 1])
...
with open(outpath_stored_model, 'wb') as file:
